...
export default function CreatorsTable({
  ...
  useEffect(() => {
    const close = (e: MouseEvent) => {
      ...
      if (target.closest('[aria-label="Actions"]')) {
        return;
      }
  ...
  return (
    <div className="space-y-3 relative table-wrapper-container">
      {/* Table */}
      <div className="overflow-hidden rounded-2xl border border-slate-800/60 bg-slate-900/20 backdrop-blur-xl shadow-xl">
                      ...
                      <button
                        onClick={(e) => {
                          ...
                          if (activeMenuId === c.id) {
                            ...
                            const container = tbodyRef.current?.closest('.table-wrapper-container');
                            if (container) {
                              const containerRect = container.getBoundingClientRect();
                              // Vertically center aligned with the button
                              const top = rect.top - containerRect.top + button.offsetHeight / 2 - 40;
                              // Position it right-aligned inside the table container's horizontal bounds so it fits on all screen sizes
                              const left = containerRect.width - 160;
      ...
      {/* Actions Dropdown Menu (outside the overflow-hidden table container) */}
      {activeMenuId && activeCreator && (
        <div
          className="absolute z-50 w-36 rounded-xl border border-slate-800/80 bg-slate-950/95 backdrop-blur-xl shadow-2xl overflow-hidden animate-scale-up"
          style={{
